...
<%--数据表格--%>
    ...
    <script type="text/javascript">
        function toPosts(uId) {
            var uName = $("#admin222").val();
            location.href="${pageContext.request.contextPath}/m/adminToLevel?uId="+uId+"&uName="+uName;
        }
        function todel(uId) {
            var uName = $("#admin222").val();
            location.href="${pageContext.request.contextPath}/m/adminToRecoveryStatus?uId="+uId+"&uName="+uName;
        }
        function deleteUser(uId) {
            var uName = $("#admin222").val();
            location.href="${pageContext.request.contextPath}/admin/deleteUser?uId="+uId+"&uName="+uName;
        }
            ...
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <div class="row">
                        ...
                        <div class="col-sm-4"><%--<label class="pull-right">共${tableCount}条</label>--%><input id="admin222" type="hidden" value="${admin222.uName}"></div>
                    </div>
                </div>
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 8%;text-align: center">用户手机号</th>
                                <th style="width: 10%;text-align: center">用户邮箱</th>
                                <th style="width: 6%;text-align: center">用户等级</th>
                                <th style="width: 6%;text-align: center">账户状态</th>
                                <th style="width: 10%;text-align: center">用户发布信息条数</th>
                                <th style="width: 57%;text-align: right">操作</th>
                            </tr>
                        </thead>
                        <tbody>
                            <c:forEach items="${user}" var="allUserS">
                                <tr style="height: 70px;">
                                    <td style="width: 3%;text-align: center">${allUserS.uId}</td>
                                    <td style="width: 6%;text-align: center">${allUserS.uName}</td>
                                    <td style="width: 8%;text-align: center">${allUserS.uTelephone}</td>
                                    <td style="width: 10%;text-align: center">${allUserS.uEmail}</td>
                                    <td style="width: 6%;text-align: center">
                                        <c:if test="${allUserS.uLevel eq '1'}">
                                            <button type="button" class="btn btn-warning del-1"  onclick="toPosts(${allUserS.uId})">降级</button>
                                        </c:if>
                                        <c:if test="${allUserS.uLevel eq '2'}">
                                            <button type="button" class="btn btn-success del-1" onclick="toPosts(${allUserS.uId})">升级</button>
                                        </c:if>
                                    </td>
                                    <td style="width: 6%;text-align: center">
                                        <c:if test="${allUserS.uRecoveryStatus eq 48}">
                                            <button type="button" class="btn btn-danger del"  onclick="todel(${allUserS.uId})">冻结账号</button>
                                        </c:if>
                                        <c:if test="${allUserS.uRecoveryStatus eq 49}">
                                            <button type="button" class="btn btn-primary del" onclick="todel(${allUserS.uId})">解封账号</button>
                                        </c:if>
                                    </td>
                                    <th style="width: 10%;text-align: center">${allUserS.uPassword}</th>
                                    <th style="width: 57%;text-align: right">
                                        <button type="button" class="btn btn-danger del"  onclick="deleteUser(${allUserS.uId})">注销该账号</button>
                                    </th>
                                </tr>
                            </c:forEach>
                        </tbody>
